...
export class ApiClient {
  ...
  private async requestMessage(
    ...
    try {
      const response = await this.client.request<
        MessageResponse | ApiEnvelope<unknown>
      >({
        ...options,
        data: options.body,
        headers: this.buildHeaders(options),
  ...
  private async ensureCsrfCookie() {
    if (!this.csrfRequest) {
      this.csrfRequest = this.client
        .get(this.originUrl(csrfCookiePath), {
          withCredentials: true,
        })
        .then(() => undefined)
        .finally(() => {
  ...
  private ensureWriteAllowed(path: string, options: RequestOptions) {
    ...
    if (
      isMutation &&
      !isAuthRequest &&
      compatibilityStore.getSnapshot().status !== "supported"
    ) {
      throw new ApiError(
        "Changes are disabled until the app version is supported.",
        {
  ...
  private syncUnsupportedVersion(envelope: ApiEnvelope<unknown>) {
    ...
    if (
      envelope.statusCode !== 426 ||
      data?.code !== "UNSUPPORTED_FRONTEND_VERSION"
    )
      return;
    ...
    compatibilityStore.setState({
      ...
      minimumSupportedVersion:
        typeof data.minimum_supported_version === "string"

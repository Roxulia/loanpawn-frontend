...
function getSavedTenant(code: string) {
  return (
    listSavedTenants().find(
      (tenant) => tenant.code === normalizeTenantCode(code),
    ) ?? null
  );
}
...
function saveTenantProfile(tenant: TenantDetail): SavedTenantProfile {
  ...
  const tenants = listSavedTenants().filter(
    (savedTenant) => savedTenant.code !== profile.code,
  );
...
function removeSavedTenant(code: string) {
  ...
  const nextTenants = listSavedTenants().filter(
    (tenant) => tenant.code !== normalizedCode,
  );
...
function getActiveTenantCode() {
  ...
  return (
    normalizeTenantCode(window.localStorage.getItem(activeTenantKey) ?? "") ||
    null
  );
}
...
function readSavedTenantProfile(value: unknown): SavedTenantProfile | null {
  ...
  return {
    ...
    licenseStatus,
    lastUsedAt:
      typeof tenant.lastUsedAt === "string"
        ? tenant.lastUsedAt

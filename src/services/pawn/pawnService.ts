...
import type {
  LoanContractSlip,
  LoanContractSlipPayload,
} from "../../dataobjects/pawn/slip";
...
export const pawnService = {
  listLoanContractSlips(auth?: TenantAuth) {
    return apiClient.get<PaginatedResult<LoanContractSlip>>(
  ...
  payInterest(
    slipNo: string,
    payload: { payment_amount: number; record_debt?: boolean },
    auth?: TenantAuth,
  ) {
  ...
  listRedemptions(auth?: TenantAuth) {
    return apiClient.get<PaginatedResult<unknown>>(
      "/tenant/redemptions",
      authOptions(auth),
    );
  },
  ...
  getRedemptionRecord(slipNumber: string, auth?: TenantAuth) {
    return apiClient.get<unknown>(
      `/tenant/redemption-records/${encodeURIComponent(slipNumber)}`,
      authOptions(auth),
    );
  },

...
import {
  ...
  formatDate,
} from "../../modules/finance/financeFormat";
...
type DashboardAmountFormatter = (
  value: string | number | null | undefined,
) => string;
type SourceAmountFormatter = (
  value: string | number | null | undefined,
...
function makeLoanAttentionColumns(
  formatAmount: SourceAmountFormatter,
): Array<DataTableColumn<DashboardLoanAttention>> {
  return [
    {
...
function makeCollateralColumns(
  formatAmount: SourceAmountFormatter,
): Array<DataTableColumn<AdjustedCollateralItem>> {
  return [
    {
      header: "Item Name",
      key: "itemName",
...
export function DashboardPage() {
  ...
  const [timeFilter, setTimeFilter] =
    useState<DashboardTimeFilter>("this_month");
  ...
  const reviewItems = useMemo(
    () =>
      adjustedCollateral
        .filter(
          (item) => item.status === "Expired" || item.status === "Low Margin",
        )
  ...
  const formatSourceAmount = useCallback(
    (value: string | number | null | undefined, currency: DashboardCurrency) =>
      formatPlainCurrencyAmount(value, currency, locale),
    [locale],
  );
  const loanAttentionColumns = useMemo(
    () => makeLoanAttentionColumns(formatSourceAmount),
    [formatSourceAmount],
  );
  const collateralColumns = useMemo(
    () => makeCollateralColumns(formatSourceAmount),
    [formatSourceAmount],
  );
  ...
  return (
    <section className="page dashboard-situation-page">
      ...
      {summary ? (
        <>
          <section
            className="dashboard-situation-kpis"
            aria-label="Shop situation summary"
          >
            <SituationKpiCard
            ...
            <SituationKpiCard
              ...
              value={
                <CurrencyAmountList
                  amounts={summary.financial.activeLoanAmounts}
                  formatAmount={formatSourceAmount}
                />
              }
            />
            <SituationKpiCard
              ...
              tone="success"
              trend={trendText(
                summary.financial.interestCollected,
                summary.financial.previousInterestCollected,
                "monthly comparison",
          ...
          >
            <div className="dashboard-financial-grid">
              <div className="dashboard-metric-grid">
                ...
                <DashboardMetric
                  label="Total Income"
                  value={formatReportingAmount(summary.financial.totalIncome)}
                />
                <DashboardMetric
                  label="Total Expenses"
                  tone="warning"
                  value={formatReportingAmount(summary.financial.totalExpenses)}
                />
                <DashboardMetric
                  label="Net Profit"
                  tone={summary.financial.netProfit >= 0 ? "success" : "danger"}
              ...
              <div className="dashboard-chart-card">
                <div className="dashboard-chart-card__header">
                  ...
                  <Badge tone="info">
                    {formatDate(summary.filters.start_at)} to{" "}
                    {formatDate(summary.filters.end_at)}
                  </Badge>
                </div>
                <LineChart
                  formatSourceAmount={formatSourceAmount}
                  points={summary.financial.chart}
                />
          ...
          >
            <div className="dashboard-risk-cards">
              <DashboardMetric
                label="Due Today"
                tone="warning"
                value={`${summary.risk.dueToday} loans`}
              />
              <DashboardMetric
                label="Due This Week"
                tone="warning"
                value={`${summary.risk.dueThisWeek} loans`}
              />
              <DashboardMetric
                label="Overdue Loans"
                tone="danger"
                value={`${summary.risk.overdueLoans} loans`}
              />
              <DashboardMetric
                ...
                value={
                  <CurrencyAmountList
                    amounts={summary.risk.overdueAmounts}
                    formatAmount={formatSourceAmount}
                  />
                }
              />
              <DashboardMetric
                ...
                tone="danger"
                value={summary.risk.highRiskCustomers}
              />
              <DashboardMetric
                label="Bad Repayment History Count"
                tone="warning"
                value={summary.risk.badRepaymentHistoryCount}
              />
            </div>
            <div className="dashboard-table-card">
              <TableHeader
                count={summary.risk.loansRequiringAttention.length}
          ...
          >
            <div className="dashboard-collateral-cards">
              <DashboardMetric
                ...
                value={
                  <CurrencyAmountList
                    amounts={adjustedCollateralSummary.totalValues}
                    formatAmount={formatSourceAmount}
                  />
                }
              />
              <DashboardMetric
                ...
                value={
                  <CurrencyRatioList
                    ratios={adjustedCollateralSummary.ltvByCurrency}
                  />
                }
              />
              <DashboardMetric
                ...
                value={
                  <CurrencyAmountList
                    amounts={adjustedCollateralSummary.jewelleryValues}
                    formatAmount={formatSourceAmount}
                  />
                }
              />
              <DashboardMetric
                label="Expired Collateral Count"
                tone="danger"
                value={adjustedCollateralSummary.expiredCount}
              />
              <DashboardMetric
                label="Low-Margin Collateral Items"
                tone="warning"
                value={adjustedCollateralSummary.lowMarginCount}
              />
            </div>
            <div className="dashboard-collateral-layout">
              <div className="dashboard-chart-card">
                ...
                <DonutChart
                  categories={adjustedCollateralSummary.categories}
                  formatAmount={formatSourceAmount}
                />
              </div>
              <div className="dashboard-table-card">
                <TableHeader
                  ...
                  title="Collateral Items Needing Review"
                />
                <DataTable
                  actions={() => (
                    <Button variant="secondary">View Detail</Button>
    ...
    <header className="dashboard-situation-header">
      ...
      <div className="dashboard-situation-controls">
        <Select
          ...
          onChange={(event) =>
            onTimeFilterChange(event.target.value as DashboardTimeFilter)
          }
          value={timeFilter}
        >
        ...
        <Button isLoading={isRefreshing} onClick={onRefresh} variant="primary">
          Refresh
        </Button>
      </div>
    </header>
...
function CurrencyAmountList({
  ...
  return (
    <span className="dashboard-currency-values">
      {amounts.map((item) => (
        <span key={currencyKey(item.currency)}>
...
function CurrencyRatioList({ ratios }: { ratios: DashboardCurrencyRatio[] }) {
  ...
  return (
    <span className="dashboard-currency-values">
      {ratios.map((item) => (
        <span key={currencyKey(item.currency)}>
          {item.currency.code || "Unknown"} · {formatPercent(item.ratio)}
        </span>
      ))}
    </span>
  );
...
function RiskBadge({
  ...
  riskLevel: DashboardLoanAttention["riskLevel"];
}) {
  const tone =
    riskLevel === "High"
      ? "danger"
...
function CollateralStatusBadge({
  ...
  status: AdjustedCollateralItem["status"];
}) {
  const tone =
    status === "Expired"
      ? "danger"
...
function LineChart({
  ...
  formatSourceAmount: SourceAmountFormatter;
  points: DashboardFinancialChartPoint[];
}) {
  if (points.length === 0) {
    return (
      <EmptyState
        description="No financial movement is available for this period."
        title="No chart data"
      />
    );
  }
  ...
  const series = [
    ...
    {
      color: "#0ea5e9",
      key: "returnedAmount" as const,
      label: "Returned Amount",
    },
    { color: "#10b981", key: "interest" as const, label: "Interest" },
    { color: "#ef4444", key: "expenses" as const, label: "Expense" },
  ];
  const loanTotals = summarizeCurrencyAmounts(
    points.flatMap((point) => point.loanAmounts),
  );
  ...
  return (
    <div className="dashboard-line-chart">
      <svg
        ...
        role="img"
        aria-label="Financial movement line chart"
      >
        <line
        ...
        {series.map((item) => (
          <polyline
            ...
            key={item.key}
            points={points
              .map((point, index) => coordinate(index, point[item.key]))
              .join(" ")}
            stroke={item.color}
            strokeLinecap="round"
...
function DonutChart({
  ...
  categories: DashboardCollateralCategory[];
  formatAmount: SourceAmountFormatter;
}) {
  ...
  if (validCategories.length === 0) {
    return (
      <EmptyState
        description="No collateral value is available yet."
        title="No collateral categories"
      />
    );
  }
  ...
  return (
    <div className="dashboard-donut-groups">
      {Array.from(groups.values()).map((group) => (
        <CurrencyDonut
          categories={group}
          formatAmount={formatAmount}
          key={currencyKey(group[0].currency)}
        />
      ))}
    </div>
  );
...
function CurrencyDonut({
  ...
  return (
    <div className="dashboard-donut">
      <strong className="dashboard-donut__currency">
        {currency.code || "Unknown currency"}
      </strong>
      <svg
        viewBox="0 0 42 42"
        role="img"
        aria-label="Collateral category donut chart"
      >
        <circle
      ...
      collateralItems
        ...
        .map((item) => {
          const material = materialTypes.find(
            (option) => option.id === item.materialTypeId,
          ) ?? {
            id: item.materialTypeId as number,
            name: item.materialTypeName ?? "Material",
          };

          return [
            materialPriceKey(material.id, item.currency),
            { currency: item.currency, material },
          ] as const;
        }),
    ).values(),
  );
  ...
  return (
    ...
    >
      <div className="dashboard-material-price-modal">
        <p>
          Enter a price per kyat for each source currency. Values are used only
          for this dashboard session.
        </p>
        {priceFields.length === 0 ? (
          <EmptyState
            description="No jewellery collateral with a source currency is available."
            title="No material prices needed"
          />
        ) : (
          priceFields.map(({ currency, material }) => (
            <FormField
              helperText={`${currency.code || "Unknown currency"} per kyat`}
              id={`material-price-${material.id}-${currencyKey(currency)}`}
              key={`${material.id}-${currencyKey(currency)}`}
              label={`${material.name} · ${currency.code || "Unknown"}`}
            >
              <Input
                id={`material-price-${material.id}-${currencyKey(currency)}`}
                min="0"
                onChange={(event) =>
                  updatePrice(material.id, currency, event.target.value)
                }
                placeholder="0"
                type="number"
                value={
                  materialPrices[materialPriceKey(material.id, currency)] ?? ""
                }
              />
            </FormField>
          ))
...
function adjustCollateralItems(
  items: DashboardCollateralReviewItem[],
  materialPrices: MaterialPriceMap,
): AdjustedCollateralItem[] {
  return items.map((item) => {
    const materialPrice =
      item.materialTypeId === null
        ...
        : Number(
            materialPrices[
              materialPriceKey(item.materialTypeId, item.currency)
            ] ?? 0,
          );
    const jewelleryWeight = item.kyat + item.pal / 16 + item.yway / 128;
    const currentMarketValue =
      item.isJewellery && materialPrice > 0 && jewelleryWeight > 0
        ? jewelleryWeight * materialPrice
        : item.estimatedMarketValue;
    const displayLtvRatio =
      currentMarketValue <= 0
        ? 0
        : (item.loanAmount / currentMarketValue) * 100;
    const status =
      item.status === "Expired"
        ? item.status
        : displayLtvRatio >= 85
...
function summarizeAdjustedCollateral(items: AdjustedCollateralItem[]) {
  const totalValues = summarizeCurrencyAmounts(
    items.map((item) => ({
      amount: item.currentMarketValue,
      currency: item.currency,
    })),
  );
  const jewelleryValues = summarizeCurrencyAmounts(
    items
      .filter((item) => item.isJewellery)
      .map((item) => ({
        amount: item.currentMarketValue,
        currency: item.currency,
      })),
  ...
  const ltvMap = new Map<
    string,
    { currency: DashboardCurrency; loanTotal: number; valueTotal: number }
  >();
  ...
  items.forEach((item) => {
    ...
    if (item.currentMarketValue > 0) {
      const currencyLtv = ltvMap.get(currencyKey(item.currency)) ?? {
        currency: item.currency,
        loanTotal: 0,
        valueTotal: 0,
      };
  ...
  return {
    ltvByCurrency: Array.from(ltvMap.values()).map((item) => ({
      currency: item.currency,
      ratio: (item.loanTotal / item.valueTotal) * 100,
    })),
...
function formatPlainCurrencyAmount(
  value: string | number | null | undefined,
  currency: DashboardCurrency,
  locale?: string,
) {
...
function trendText(
  ...
  previous: number,
  suffix: string,
  formatAmount: DashboardAmountFormatter,
) {

...
export function Select({
  ...
  const options = useMemo(
    () => getSelectOptions(translatedChildren),
    [translatedChildren],
  );
  ...
  const updateMenuPosition = useCallback(() => {
    ...
    const left = Math.min(
      ...
      Math.max(
        SELECT_MENU_VIEWPORT_PADDING,
        viewportWidth - minWidth - SELECT_MENU_VIEWPORT_PADDING,
      ),
    );
    const maxWidth = Math.max(
      minWidth,
      viewportWidth - left - SELECT_MENU_VIEWPORT_PADDING,
    );
    const spaceBelow =
      viewportHeight -
      rect.bottom -
      ...
      SELECT_MENU_VIEWPORT_PADDING;
    const spaceAbove =
      rect.top - SELECT_MENU_GAP - SELECT_MENU_VIEWPORT_PADDING;
    ...
    setMenuPosition({
      bottom: openAbove
        ? viewportHeight - rect.top + SELECT_MENU_GAP
        : undefined,
      left,
  ...
  return (
    <div
      className={["ui-select-combobox", className].filter(Boolean).join(" ")}
      ref={wrapperRef}
    >
      <select
        aria-hidden="true"

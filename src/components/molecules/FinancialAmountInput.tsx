...
import {
  useFinancialUnits,
  type FinancialAmountValue,
  type FinancialUnit,
} from "../../modules/finance/financialUnits";
...
export function FinancialAmountInput(props: Props) {
  ...
  return (
    <>
      ...
      {error && (
        <small className="financial-amount-input__error" role="alert">
          {t("Unable to load financial units.")}
        </small>
      )}
    </>
  );
...
function FinancialAmountInputDesktop(props: PresentationProps) {
  return (
    <FinancialAmountFields
      {...props}
      className="financial-amount-input financial-amount-input--desktop"
    />
  );
}
...
function FinancialAmountInputMobile(props: PresentationProps) {
  return (
    <FinancialAmountFields
      {...props}
      className="financial-amount-input financial-amount-input--mobile"
    />
  );
}
...
function FinancialAmountFields({
  ...
  className,
  ...rest
}: PresentationProps & { className: string }) {
  return (
    <div className={className}>
      ...
      <Select
        ...
        onChange={(event) =>
          onChange({
            ...value,
            unit: event.target.value as FinancialAmountValue["unit"],
          })
        }
        value={value.unit}
      >

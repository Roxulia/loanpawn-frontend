...
export function NrcField({
  ...
  const townships = selectedRegion
    ? Object.entries(selectedRegion.townships)
    : [];

  function updateState(nextState: string) {
    const nextRegion = nrcRegions.find(
      (region) => region.code_en === nextState,
    );
    const nextTownship =
      nextRegion && value.township in nextRegion.townships
        ? value.township
        : "";
  ...
  function updateNumber(nextNumber: string) {
    const digits =
      nextNumber.match(nrcInputDigitPattern)?.join("").slice(0, 6) ?? "";
  ...
  return (
    <div className="nrc-field">
      ...
      <span className="nrc-field__separator" aria-hidden="true">
        /
      </span>
      <Select
        ...
        id={`${id}-township`}
        onChange={(event) =>
          onChange({ ...value, township: event.target.value })
        }
        required={required}
      ...
      <Select
        ...
        id={`${id}-citizen`}
        onChange={(event) =>
          onChange({ ...value, citizen: event.target.value })
        }
        required={required}
        value={value.citizen}
      >
...
export function isValidNrcNumber(value: string) {
  return (
    nrcDigitPattern.test(value) &&
    /^[0-9]{6}$/.test(normalizeMyanmarDigits(value))
  );
}

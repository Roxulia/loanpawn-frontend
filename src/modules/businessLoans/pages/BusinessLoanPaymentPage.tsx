...
const paymentColumns: Array<DataTableColumn<BusinessLoanPayment>> = [
  ...
  { header: "Interest", key: "interest", render: (row) => row.interest_paid },
  {
    header: "Allocation",
    key: "allocation",
    render: (row) => row.allocation_order.replace("_", " "),
  },
  {
    header: "Paid at",
    key: "date",
...
export function BusinessLoanPaymentPage() {
  ...
  const [history, setHistory] = useState<BusinessLoanPayment[]>([]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      await businessLoanService.pay(
        loanCode,
        {
          amount_paid: paymentAmount,
          amount_paid_unit: calculation.allow_partial_payments ? unit : "UNIT",
          allocation_order: order,
          loan_update_key: calculation.loan_update_key,
          payment_account_id: accountId ? Number(accountId) : undefined,
          reporting_exchange_rate: rate ? Number(rate) : undefined,
          reporting_exchange_rate_inversed: inversed,
        },
        createIdempotencyKey(),
      );
      setNotice("Business loan payment recorded.");
      setAmount("");
      await load();
    } catch (reason) {
      setError(
  ...
  return (
    <section className="page business-loan-payment-page">
      <SectionHeader
        title="Business Loan Payment"
        subtitle={loanCode}
        action={
          <Button
            onClick={() => navigate(routePaths.businessLoanDetail(loanCode))}
            variant="secondary"
          >
            Back
          </Button>
        ...
        calculation && (
          <div className="business-loan-payment-layout">
            ...
            <Card
              ...
              description="Allocate a payment against this business loan."
            >
              <form className="ui-form" onSubmit={submit}>
                <FormGroup columns={1}>
                  {calculation.allow_partial_payments ? (
                    <FormField id="business-loan-payment-amount" label="Amount">
                      <FinancialAmountInput
                        id="business-loan-payment-amount"
                        min="0.01"
                        onChange={(next) => {
                  ...
                  <FormField
                    id="business-loan-payment-order"
                    label="Allocation"
                  >
                    <Select
                      id="business-loan-payment-order"
                  ...
                  <FormField
                    id="business-loan-payment-account"
                  ...
                  <ReportingExchangeRateField
                    accountId={accountId || String(calculation.account_id)}
                    inversed={inversed}
                    manualRate={rate}
                    onInversedChange={setInversed}
                    onManualRateChange={setRate}
                  />
                </FormGroup>
                <ActionBar>
                  <Button
                    disabled={
                      calculation.allow_partial_payments && Number(amount) <= 0
                    }
                    isLoading={loading}
                    type="submit"
                  >
            ...
            <Card
              ...
              description="Payments recorded against this loan."
            >
              <DataTable

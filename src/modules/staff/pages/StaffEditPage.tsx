...
export function StaffEditPage() {
  ...
  const hasPotentialEditAccess =
    canEditOwnProfile || canEditRegularStaff || canEditAdmin;
  ...
  const canManageRole =
    ...
    !isOwnerTarget &&
    canEditTarget &&
    hasPermission("update_user_roles");
  ...
  const canManageFinancialAccounts =
    ...
    !isOwnerTarget &&
    canEditTarget &&
    hasPermission("manage_financial_account_assignments");
  ...
  const [financialAccounts, setFinancialAccounts] = useState<
    FinancialAccount[]
  >([]);
  const [selectedFinancialAccountIds, setSelectedFinancialAccountIds] =
    useState<number[]>([]);
  ...
  const [isLoadingFinancialAccounts, setIsLoadingFinancialAccounts] =
    useState(false);
  const [isSavingFinancialAccounts, setIsSavingFinancialAccounts] =
    useState(false);
      ...
      try {
        ...
        const nextFormWithRole = {
          ...
          role_id:
            ...
            !roleOptions.some(
              (role) => String(role.role_id) === nextForm.role_id,
            )
              ? ""
              : nextForm.role_id,
        };
        ...
        const loadedPermissions = (response.permissions ??
          []) as PermissionCode[];
        setSelectedPermissions(loadedPermissions);
        setInitialPermissions(loadedPermissions);
        setSelectedFinancialAccountIds(
          (response.financial_accounts ?? []).map((account) => account.id),
        );
        setFinancialAccounts((current) => {
          const knownIds = new Set(current.map(({ id }) => id));
          return [
            ...current,
            ...((response.financial_accounts ?? []).filter(
              ({ id }) => !knownIds.has(id),
            ) as FinancialAccount[]),
          ];
        });
  ...
  const loadRoleOptions = useCallback(async () => {
    ...
    try {
      ...
      setForm((current) => ({
        ...
        role_id:
          current.role_id &&
          roles.some((role) => String(role.role_id) === current.role_id)
            ? current.role_id
            : "",
      }));
  ...
  function handleFinancialAccountToggle(accountId: number) {
    setSelectedFinancialAccountIds((current) =>
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await staffService.updateUser(
        staffCode,
        formToStaffPayload(form, { includeRole: canManageRole }),
      );
      navigate(routePaths.staff, {
        state: { notice: "Staff account updated." },
      });
    } catch (saveError) {
      setPageError(
  ...
  async function handleFinancialAccountSave() {
    ...
    setNotice(null);
    try {
      const response = await staffService.updateFinancialAccountAssignments(
        staffCode,
        selectedFinancialAccountIds,
      );
      setSelectedFinancialAccountIds(
        response.financial_accounts.map((account) => account.id),
      );
      ...
      setPageError(
        saveError instanceof Error
          ? saveError.message
          : "Unable to update financial account access.",
      );
    } finally {
  ...
  async function handleResetPasswordToDefault() {
    ...
    try {
      const response = await staffService.resetPasswordToDefault(staffCode, {
        logoutFromAll: true,
      });
      setIsResetConfirmOpen(false);
      const resetMessage =
        response?.message || "Password reset to tenant default.";
        ...
        <>
          <StaffForm
            ...
            isRoleOptionDisabled={(role) => {
              ...
              if (!canManageRole || role.role_name.toLowerCase() === "owner")
                return true;

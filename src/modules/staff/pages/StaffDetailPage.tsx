...
export function StaffDetailPage() {
  ...
  const targetRoleName = staffUser
    ? getUserRoleName(staffUser).toLowerCase()
    : "";
      ...
      {isLoading ? (
        ...
        <>
          <Card
            ...
            action={
              <Badge
                tone={staffUser.status === "active" ? "success" : "warning"}
              >
                {staffUser.status}
              </Badge>
            }
          ...
          <PermissionToggleForm readOnly value={selectedPermissions} />

          <FinancialAccountAssignmentForm
            accounts={staffUser.financial_accounts ?? []}
            readOnly
            selectedAccountIds={(staffUser.financial_accounts ?? []).map(
              (account) => account.id,
            )}
          />
        </>

...
export type StaffFinancialAccountAssignmentResponse = {
...
export const staffService = {
  ...
  listRoles(params: { excludeOwner?: boolean } = {}) {
    ...
    return apiClient.get<TenantRoleOption[]>(
      `/tenant/user-roles${query ? `?${query}` : ""}`,
    );
  },
  ...
  getUser(userCode: string) {
    return apiClient.get<TenantUser>(
      `/tenant/users/${encodeURIComponent(userCode)}`,
    );
  },
  ...
  updatePermissions(userCode: string, payload: StaffPermissionPayload) {
    return apiClient.put<TenantUser>(
      `/tenant/users/${encodeURIComponent(userCode)}/permissions`,
      payload,
    );
  },

  updateFinancialAccountAssignments(
    userCode: string,
    financialAccountIds: number[],
  ) {
    return apiClient.put<StaffFinancialAccountAssignmentResponse>(
      `/tenant/users/${encodeURIComponent(userCode)}/financial-account-assignments`,
      {
  ...
  resetPasswordToDefault(
    userCode: string,
    payload: { logoutFromAll?: boolean } = {},
  ) {
    return apiClient.putMessage(
      `/tenant/users/${encodeURIComponent(userCode)}/reset-to-defaultpassword`,
      payload,
    );
  },

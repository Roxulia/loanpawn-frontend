...
function LoadedExpenseDetailModal({
  expense,
  onClose,
}: {
...
function ExpenseDetailDesktop({
  ...
  return (
    <section
      aria-modal="true"
      className="expense-detail-desktop"
...
function ExpenseDetailMobile({
  ...
  return (
    <section
      aria-modal="true"
      className="expense-detail-mobile"
      onMouseDown={(event) => event.stopPropagation()}
      role="dialog"
    >
      <ExpenseDetailHeader expense={expense} onClose={onClose} />
...
function ExpenseDetailHeader({
  expense,
  onClose,
}: Pick<DetailPresentationProps, "expense" | "onClose">) {
  return (
...
function ExpenseDetailContent({ expense }: { expense: TenantExpense }) {
  ...
  const expenseType = getStringField(
    expense,
    "expense_type_name",
    "expenseTypeName",
  );
    ...
    <div className="expense-detail-content">
      <div className="expense-detail-content__image">
        {imageUrl ? (
          ...
          <div className="expense-detail-content__image-empty">
            No reference image
          </div>
        )}
      </div>
      <dl className="expense-detail-content__metadata">
        ...
        <DetailField
          label="Created"
          value={formatDate(getStringField(expense, "created_at", "createdAt"))}
        />
        <DetailField
          label="Updated"
          value={formatDate(getStringField(expense, "updated_at", "updatedAt"))}
        />

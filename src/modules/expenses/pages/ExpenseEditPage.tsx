...
export function ExpenseEditPage() {
      ...
      .then(([expense, types]) => {
        ...
        setForm({
          ...
          expense_type_id: String(
            getNumberField(expense, "expense_type_id", "expenseTypeId") ?? "",
          ),
  ...
  function updateField<TField extends keyof ExpenseFormState>(
    field: TField,
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate(routePaths.expenses, {
        state: { notice: "Expense updated successfully." },
      });
    } catch (saveError) {
      setError(
  ...
  return (
    <section className="page expense-edit-page">
      <SectionHeader
        title="Edit Expense"
        subtitle="Update expense details and reference information."
      />
      {isLoading ? (
        <LoadingState rows={5} />
      ) : (

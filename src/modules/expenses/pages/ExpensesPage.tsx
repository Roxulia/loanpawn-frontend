...
const columns: Array<DataTableColumn<TenantExpense>> = [
  ...
  },
  {
    ...
    key: "expenseType",
    render: (item) => {
      const typeName = getStringField(
        item,
        "expense_type_name",
        "expenseTypeName",
      );
      const typeCode = getStringField(
        item,
        "expense_type_code",
        "expenseTypeCode",
      );
...
const config: FinanceResourcePageConfig<TenantExpense, ExpenseForm> = {
  ...
  deleteMessage: (item) =>
    `Delete expense "${item.description}"? This action cannot be undone.`,
  ...
  itemToForm: (item) => ({
    ...
    expense_type_id: String(
      getNumberField(item, "expense_type_id", "expenseTypeId") ?? "",
    ),
    has_existing_image: Boolean(
      item.has_image_reference ?? item.hasImageReference,
    ),
    image_reference: null,
    remove_image_reference: false,
    reporting_exchange_rate: "",
  ...
  save: (mode, form, item) => {
    ...
    payload.set(
      "update_key",
      String(getNumberField(item ?? {}, "update_key", "updateKey") ?? 0),
    );
...
export function ExpensesPage() {
  const [detailExpense, setDetailExpense] = useState<TenantExpense | null>(
    null,
  );
      ...
      <FinanceResourcePage
        config={{
          ...
          renderItemActions: (item) => (
            <Button onClick={() => setDetailExpense(item)} variant="secondary">
              View
            </Button>
          ),
    ...
    <FormGroup columns={2}>
      <FormField id="expense-account" label="Payment Account">
        <FinancialAccountSelect
          id="expense-account"
          locked={isEditing}
          onChange={(accountId) => updateField("account_id", accountId)}
          value={form.account_id}
        />
        {isEditing ? (
          <span className="ui-form-field__hint">
            The account used by a posted expense cannot be changed.
          </span>
        ) : null}
      </FormField>
      <ReportingExchangeRateField
        accountId={form.account_id}
        inversed={form.reporting_exchange_rate_inversed}
        manualRate={form.reporting_exchange_rate}
        onInversedChange={(inversed) =>
          updateField("reporting_exchange_rate_inversed", inversed)
        }
        onManualRateChange={(rate) =>
          updateField("reporting_exchange_rate", rate)
        }
      />
      ...
      >
        <Select
          ...
          id="expense-type-id"
          onChange={(event) =>
            updateField("expense_type_id", event.target.value)
          }
      ...
      >
        <ExpenseImageInput
          ...
          onRemoveChange={(removed) =>
            updateField("remove_image_reference", removed)
          }
        />
      </FormField>
      <div className="ui-form-field">
        <span className="ui-label">
          <LocalizedText text="Accounting effect" />
        </span>
        <Badge tone="warning">Outgoing</Badge>
        <div className="ui-form-field__hint">
          <LocalizedText text="Expense records create outgoing accounting entries server-side." />
        </div>
      </div>
    </FormGroup>
  );
...
function validate(form: ExpenseForm) {
  ...
  if (
    form.image_reference &&
    !["image/jpeg", "image/png", "image/webp"].includes(
      form.image_reference.type,
    )
  ) {

...
export function FinancialAccountCreatePage() {
  ...
  useEffect(() => {
    Promise.all([
      financialAccountService.accountTypes(),
      financialAccountService.currencies(),
    ])
      .then(([typePage, currencyPage]) => {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      navigate(routePaths.financialAccounts, {
        state: { notice: "Financial account created." },
      });
    } catch (reason) {
      setError(
        reason instanceof Error
  ...
  return (
    <section className="page">
      <SectionHeader
        ...
        subtitle="Add another cash, bank, or online payment account."
      />
      <FinancialAccountForm
        value={form}
        accountTypes={types}

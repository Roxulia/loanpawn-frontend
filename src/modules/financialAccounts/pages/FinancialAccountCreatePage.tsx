...
export function FinancialAccountCreatePage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setSaving(true); setError(null); try { await financialAccountService.create({ account_type: form.account_type, currency_type: form.currency_type, account_name: form.account_name.trim(), balance: form.balance === '' ? 0 : Number(form.balance), balance_unit: form.balance_unit, allow_negative_balance: form.allow_negative_balance, account_number: form.account_number.trim() || null }); navigate(routePaths.financialAccounts, { state: { notice: 'Financial account created.' } }) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to create financial account.') } finally { setSaving(false) } }

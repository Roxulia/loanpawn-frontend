...
export function FinancialAccountListPage() {
  ...
  return <section className="page"><SectionHeader title="Financial Accounts" subtitle="Manage cash, bank, and online payment balances." action={hasPermission('create_financial_account') ? <Button leftIcon={<CirclePlusIcon />} onClick={() => navigate(routePaths.financialAccountCreate)} variant="primary">Add Account</Button> : null} /><Card title="Accounts" description={`${page.total} financial account${page.total === 1 ? '' : 's'}`}>{error && <Alert message={error} onDismiss={() => setError(null)} title="Account action failed" tone="danger" />}{notice && <Alert message={notice} onDismiss={() => setNotice(null)} title="Accounts updated" tone="success" />}<TableToolbar actions={<Button onClick={() => void load()}>Refresh</Button>} search={<SearchField id="financial-account-search" label="Search accounts" placeholder="Code, name, or account number" value={search} onChange={(event) => { setSearch(event.target.value); setPageNumber(1) }} />} /><DataTable items={page.items} columns={columns} isLoading={loading} getItemId={(item) => item.id} getItemTitle={(item) => item.account_name} emptyTitle="No financial accounts" pagination={{ currentPage: page.current_page, lastPage: page.last_page, total: page.total, onPrevious: () => setPageNumber((current) => Math.max(1, current - 1)), onNext: () => setPageNumber((current) => Math.min(page.last_page, current + 1)) }} actions={(item) => <div className="row-actions">{hasPermission('update_financial_account') && <Button aria-label={`Edit ${item.account_name}`} className="ui-button--icon" onClick={() => navigate(routePaths.financialAccountEdit(item.account_code))}><EditIcon /></Button>}{hasPermission('delete_financial_account') && !item.is_default && <Button aria-label={`Delete ${item.account_name}`} className="ui-button--icon" variant="danger" onClick={() => setTarget(item)}><TrashIcon /></Button>}</div>} /></Card><ConfirmDialog title="Delete financial account" message={`Delete ${target?.account_name ?? 'this account'}? Transaction history will be retained.`} confirmLabel="Delete" isOpen={Boolean(target)} isLoading={deleting} onCancel={() => setTarget(null)} onConfirm={() => void remove()} /></section>
}

...
const config: FinanceResourcePageConfig<TenantCapital, CapitalForm> = {
  ...
  deleteMessage: (item) =>
    `Delete capital "${item.description}"? This action cannot be undone.`,
  ...
  getSearchText: (item) => [item.description, item.amount].join(" "),
    ...
    <FormGroup columns={2}>
      <FormField id="capital-account" label="Financial Account">
        ...
        {isEditing ? (
          <span className="ui-form-field__hint">
            The account used by posted capital cannot be changed.
          </span>
        ) : null}
      </FormField>
      <FormField error={errors.amount} id="capital-amount" label="Amount">
        <FinancialAmountInput
          ...
          min="0.01"
          onChange={(next) => {
            updateField("amount", next.amount);
            updateField("amount_unit", next.unit);
          }}
          step="0.01"
      ...
      <ReportingExchangeRateField
        ...
        onInversedChange={(value) =>
          updateField("reporting_exchange_rate_inversed", value)
        }
        onManualRateChange={(value) =>
          updateField("reporting_exchange_rate", value)
        }
      />
      <div className="ui-form-field">
        ...
        <div className="ui-form-field__hint">
          <LocalizedText text="Capital records create incoming accounting entries server-side." />
        </div>
      </div>
      <FormField
...
export function capitalPayload(form: CapitalForm) {
  return {
    ...
    ...(form.reporting_exchange_rate
      ? {
          reporting_exchange_rate: Number(form.reporting_exchange_rate),
          reporting_exchange_rate_inversed:
            form.reporting_exchange_rate_inversed,
        }
      : {}),
  };
}

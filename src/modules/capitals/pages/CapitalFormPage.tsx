...
function CapitalFormPage({ mode }: { mode: "create" | "edit" }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void tenantResourceService
      .getCapital(capitalCode)
      .then((item) => {
        if (!active) return;
        setCapital(item);
        setForm({
  ...
  async function save() {
    ...
    try {
      if (mode === "create")
        await tenantResourceService.createCapital(capitalPayload(form));
      else
        await tenantResourceService.updateCapital(capitalCode, {
          ...capitalPayload(form),
          update_key:
            getNumberField(capital ?? {}, "update_key", "updateKey") ?? 0,
        });
      navigate(routePaths.capitals, {
        state: {
          notice: `Capital ${mode === "create" ? "created" : "updated"} successfully.`,
        },
      });
    } catch (reason) {
      setError(
  ...
  return (
    <section className="page capital-form-page capital-form-page--mobile">
      <SectionHeader
        title={mode === "create" ? "Add Capital" : "Edit Capital"}
        subtitle="Record capital against an assigned financial account."
      />
      {error && (
        <Alert
          message={error}
          onDismiss={() => setError(null)}
          title="Capital action failed"
          tone="danger"
        />
      )}
      <Card title="Capital details">
        {loading ? (
          ...
          >
            ...
            <ActionBar>
              <Button
                onClick={() => navigate(routePaths.capitals)}
                variant="secondary"
              >
                Cancel
              </Button>
              <Button isLoading={saving} type="submit" variant="primary">
                {mode === "create" ? "Add Capital" : "Save Changes"}
              </Button>
            </ActionBar>
          </form>
        )}

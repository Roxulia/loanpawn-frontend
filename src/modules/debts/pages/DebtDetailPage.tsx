...
export function DebtDetailPage() {
  ...
  return (
    <section className="page debt-detail-page">
      <SectionHeader
        title="Debt Detail"
        subtitle={debtCode}
        action={
          <div className="row-actions">
            <Button
              onClick={() => navigate(routePaths.debts)}
              variant="secondary"
            >
              Back
            </Button>
            {debt && !debt.is_paid && hasPermission("update_debt") && (
              <Button
          ...
          {!debt.is_paid &&
            ...
            (canManageSchedule || canCompound) && (
              <Card
                ...
                description="Capitalize outstanding interest into the debt principal."
              >
                <div className="debt-compounding--desktop">
                  <DebtCompoundingForm
                ...
                <div className="debt-compounding--mobile">
                  <DebtCompoundingForm
                    canCompound={Boolean(canCompound)}
                    canManageSchedule={Boolean(canManageSchedule)}
                    compoundEvery={compoundEvery}
...
function DebtDetailDesktop({
  calculation,
  debt,
}: {
  calculation: DebtInterestCalculation;
  debt: TenantDebt;
}) {
  return (
    <div className="debt-detail-summary--desktop">
      <DebtSummary calculation={calculation} debt={debt} />
    </div>
  );
}

function DebtDetailMobile({
  calculation,
  debt,
}: {
  calculation: DebtInterestCalculation;
  ...
  return (
    <div className="debt-detail-summary--mobile">
      <DebtSummary calculation={calculation} debt={debt} />
    </div>
  );
}
...
function DebtSummary({
  ...
  return (
    <Card
      ...
      action={
        <Badge tone={debt.is_paid ? "success" : "warning"}>
      ...
      <KeyValueList
        items={[
          ...
          },
          {
            ...
            value:
              (debt.compound_schedule_enabled ?? debt.compoundScheduleEnabled)
                ? `${debt.compound_every ?? debt.compoundEvery} ${debt.compound_every_type ?? debt.compoundEveryType}`
                : "Disabled",
          },
          {
            key: "Last compounded",
            value: formatDate(debt.last_compounded_at ?? debt.lastCompoundedAt),
          },
          {
            key: "Created",
            value: formatDate(getStringField(debt, "created_at", "createdAt")),
          },
          {
            key: "Updated",
            value: formatDate(getStringField(debt, "updated_at", "updatedAt")),
          },
        ]}
      />
    </Card>

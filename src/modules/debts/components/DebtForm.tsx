...
type DebtFormFieldsProps = {
  ...
  onChange: <TField extends keyof DebtFormState>(
    field: TField,
    value: DebtFormState[TField],
  ) => void;
  value: DebtFormState;
};
...
export function DebtFormFields(props: DebtFormFieldsProps) {
  ...
  return (
    <FormGroup columns={2}>
      {debtFields(props, interestTypes, "debt")}
    </FormGroup>
  );
}
...
type CreateFieldsProps = DebtFormFieldsProps & {
  interestTypes: InterestType[];
};

function DebtCreateDesktopFields(props: CreateFieldsProps) {
  return (
    <div className="debt-create-fields debt-create-fields--desktop">
      <DebtCreateSections idPrefix="debt-create-desktop" {...props} />
    </div>
  );
}
...
function DebtCreateMobileFields(props: CreateFieldsProps) {
  return (
    <div className="debt-create-fields debt-create-fields--mobile">
      <DebtCreateSections idPrefix="debt-create-mobile" mobile {...props} />
    </div>
  );
}

function DebtCreateSections({
  errors,
  idPrefix,
  interestTypes,
  mobile = false,
  onChange,
  value,
}: CreateFieldsProps & { idPrefix: string; mobile?: boolean }) {
  return (
    <>
      <FormGroup
        className="debt-create-section debt-create-section--financial"
        columns={mobile ? 1 : 2}
        description="Set the debt value and the account that records it."
      ...
      >
        ...
        <ReportingExchangeRateField
          accountId={value.created_account_id}
          inversed={value.reporting_exchange_rate_inversed}
          manualRate={value.reporting_exchange_rate}
          onInversedChange={(inversed) =>
            onChange("reporting_exchange_rate_inversed", inversed)
          }
          onManualRateChange={(rate) =>
            onChange("reporting_exchange_rate", rate)
          }
        />
      </FormGroup>
      <FormGroup
        className="debt-create-section debt-create-section--reference"
        columns={mobile ? 1 : 2}
        description="Optionally connect the debt to a slip or customer and explain why it was created."
        title="Debt Reference"
      >
      ...
      <FormGroup
        className="debt-create-section debt-create-section--interest"
        columns={mobile ? 1 : 2}
        description="Apply fixed recurring interest only when this debt requires it."
...
function debtFields(
  { errors, onChange, value }: DebtFormFieldsProps,
  interestTypes: InterestType[],
  prefix: string,
) {
  return (
    <>
      ...
      <ReportingExchangeRateField
        ...
        onManualRateChange={(rate) => onChange("reporting_exchange_rate", rate)}
      />
...
function amountField(
  errors: DebtFormErrors,
  onChange: DebtFormFieldsProps["onChange"],
  value: DebtFormState,
  id: string,
) {
  return (
    <FormField error={errors.amount} id={id} label="Amount">
      <FinancialAmountInput
        ...
        value={{
          amount: value.amount,
          unit: value.amount_unit as import("../../finance/financialUnits").FinancialUnitCode,
        }}
      />
    </FormField>
  );
...
function accountField(
  ...
  return (
    <FormField
      error={errors.created_account_id}
      helperText="The debt is recorded in this account's currency."
      id={id}
      label="Created Account"
    >
      <FinancialAccountSelect
        hasError={Boolean(errors.created_account_id)}
        id={id}
        onChange={(accountId) => onChange("created_account_id", accountId)}
        value={value.created_account_id}
      />
    </FormField>
  );
}
...
function linkField(
  ...
  return (
    <FormField
      error={errors.slip_code ?? errors.customer_code}
      helperText="Optional link for this debt."
      id={id}
      label="Debt link"
    >
      <div className="debt-link-field">
        ...
        {value.link_mode === "customer" ? (
          ...
          <Input
            hasError={Boolean(errors.slip_code)}
            id={`${id}-slip`}
            onChange={(event) => onChange("slip_code", event.target.value)}
            type="text"
            value={value.slip_code}
          />
        )}
      </div>
    </FormField>
...
function tagField(
  ...
  return (
    <FormField
      error={errors.tag}
      helperText="Example: unpaid_interest"
      id={id}
      label="Tag"
    >
      <Input
...
function descriptionField(
  ...
  return (
    <FormField error={errors.description} id={id} label="Description">
      <Textarea
...
function interestFields(
  ...
  return (
    <>
      <FormField id={`${id}-apply`} label="Interest">
        <label className="debt-interest-toggle">
          <input
            ...
            type="checkbox"
          />
          <span>Apply fixed interest from the creation date</span>
        </label>
      </FormField>
      {value.apply_interest && (
        <div className="debt-interest-fields">
          <FormField
          ...
          >
            <Input
              hasError={Boolean(errors.interest_rate)}
              id={`${id}-rate`}
              min="0.01"
              onChange={(event) =>
          ...
          <FormField
            error={errors.interest_type_id}
            id={`${id}-type`}
            label="Interest Type"
          >
            <Select
              hasError={Boolean(errors.interest_type_id)}
              id={`${id}-type`}
              onChange={(event) =>

...
export function DebtPaymentWorkflow({
  initialDebtCode = "",
}: {
  initialDebtCode?: string;
}) {
  ...
  const loadDebt = useCallback(async (code: string) => {
    ...
    try {
      ...
      setHistory([]);
      setError(
        loadError instanceof Error ? loadError.message : "Unable to load debt.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="debt-interest-workflow">
      {error && (
        <Alert
          message={error}
          onDismiss={() => setError(null)}
          title="Debt payment failed"
          tone="danger"
        />
      )}
      {notice && (
      ...
      <Card title="Debt Lookup">
        <form
          className="inline-form debt-interest-lookup"
          onSubmit={(event) => {
            event.preventDefault();
            void loadDebt(debtCode);
          }}
        >
          <FormField id="debt-interest-code" label="Debt Code">
            <Input
              id="debt-interest-code"
              onChange={(event) => setDebtCode(event.target.value)}
              value={debtCode}
            />
          </FormField>
      ...
      {calculation && (
        <div className="debt-interest-workflow__content">
          <Card title="Debt Balance">
            ...
            />
            <div className="debt-interest-accruals--desktop">
              <DataTable
            ...
            <div className="debt-interest-accruals--mobile">
              {calculation.interest_breakdown.map((row) => (
                <article key={row.id}>
                  <strong>
                  ...
                  <span>Compounded: {row.compounded_amount}</span>
                  <span>Outstanding: {row.outstanding_amount}</span>
                </article>
              ))}
            </div>
          </Card>
            ...
            >
              <form className="ui-form" onSubmit={(event) => void pay(event)}>
                <FormGroup columns={1}>
                  ...
                  {calculation.allow_partial_payments && (
                    <>
                      <FormField id="debt-payment-order" label="Payment Order">
                        <Select
                  ...
                  <FormField
                    id="debt-workflow-account"
                    label="Accepting Account"
                    helperText="Only accounts using the debt currency are shown."
                  >

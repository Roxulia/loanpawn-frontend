...
export function TemplateEditorPage() {
  ...
  const [initialHeaderLayout, setInitialHeaderLayout] =
    useState<SlipDocumentLayout>(emptyLayout);
  const [initialFooterLayout, setInitialFooterLayout] =
    useState<SlipDocumentLayout>(emptyLayout);
  const [headerLayout, setHeaderLayout] =
    useState<SlipDocumentLayout>(emptyLayout);
  const [footerLayout, setFooterLayout] =
    useState<SlipDocumentLayout>(emptyLayout);
  ...
  const activeInitialLayout =
    activeTab === "header" ? initialHeaderLayout : initialFooterLayout;
  const components = normalizeComponents(activeLayout);
  const selectedComponent =
    selectedIndex === null ? null : (components[selectedIndex] ?? null);
  const paper =
    paperPresets.find((preset) => preset.type === paperType) ?? paperPresets[0];

  const hasChanges = useMemo(() => {
    return (
      stringifyLayout(headerLayout) !== stringifyLayout(initialHeaderLayout) ||
  ...
  const loadLayouts = useCallback(async () => {
    ...
    try {
      ...
      const header = normalizeLayout(
        response.slip_header_layout ?? emptyLayout,
      );
      const footer = normalizeLayout(
        response.slip_footer_layout ?? emptyLayout,
      );
      const templateExists =
        hasLayoutComponents(header) || hasLayoutComponents(footer);
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Unable to load template layouts.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  async function saveLayouts() {
    ...
    try {
      ...
      const nextHeader = normalizeLayout(
        response.slip_header_layout ?? headerLayout,
      );
      const nextFooter = normalizeLayout(
        response.slip_footer_layout ?? footerLayout,
      );
  ...
  async function previewLayouts() {
    ...
    try {
      const html = await settingsService.previewDummySlipDocument(
        dummySlipNo,
        paperType,
      );
  ...
  function addComponent(type: ComponentType) {
    const nextComponents = [
      ...components,
      createComponent(type, 8, 8 + components.length * 8),
    ];

    setActiveLayout({
      ...activeLayout,
      version: activeLayout.version ?? 1,
  ...
  function updateSelectedProps(patch: Record<string, unknown>) {
    ...
    updateSelectedComponent({
      props: { ...selectedComponent.props, ...patch },
    });
  }
  ...
  function removeSelectedComponent() {
    ...
    setActiveLayout({
      ...activeLayout,
      components: components.filter((_, index) => index !== selectedIndex),
    });
    setSelectedIndex(null);
  }
    ...
    return (
      <section className="page">
        <SectionHeader
          ...
          subtitle="Slip document header and footer layout controls."
        />
        <LoadingState rows={6} />
      </section>
    );
  ...
  return (
    <section className="page">
      ...
      <Modal
        footer={
          <Button onClick={() => setPreviewHtml(null)} variant="secondary">
            Close
          </Button>
        }
      ...
      >
        <iframe
          ...
          srcDoc={previewHtml ?? ""}
          title="Dummy slip document preview"
        />
      </Modal>
      ...
      {!hasTemplate ? (
        <Card
          ...
          description="Create a slip document template before editing header and footer layout sections."
        >
          <ActionBar>
            <Button
        ...
        <div className="workflow-stack">
          <div className="template-editor-toolbar">
            ...
            >
              <Button
                ...
                variant={activeTab === "header" ? "primary" : "secondary"}
              >
                Header Editor
              </Button>
              <Button
                ...
                variant={activeTab === "footer" ? "primary" : "secondary"}
              >
                Footer Editor
              </Button>
            </div>
          ...
          <div className="template-editor-shell">
            ...
            <Card
              title={`${activeTab === "header" ? "Header" : "Footer"} Canvas`}
              description={`${paper.label} - ${paper.widthMm}mm x ${paper.heightMm}mm`}
            >
              ...
              {selectedComponent ? (
                <div className="template-property-grid">
                  <FormField id="template-component-x" label="X (mm)">
                    <Input
                      id="template-component-x"
                      min={0}
                      type="number"
                      value={numberValue(selectedComponent.props?.x, 0)}
                      onChange={(event) =>
                        updateSelectedProps({ x: Number(event.target.value) })
                      }
                    />
                  </FormField>
                  <FormField id="template-component-y" label="Y (mm)">
                    <Input
                      id="template-component-y"
                      min={0}
                      type="number"
                      value={numberValue(selectedComponent.props?.y, 0)}
                      onChange={(event) =>
                        updateSelectedProps({ y: Number(event.target.value) })
                      }
                    />
                  </FormField>
                  <FormField id="template-component-width" label="Width (%)">
                    <Input
                      id="template-component-width"
                      min={10}
                      max={100}
                      type="number"
                      value={numberValue(
                        selectedComponent.style?.width_percent,
                        100,
                      )}
                      onChange={(event) =>
                        updateSelectedStyle({
                  ...
                  <FormField
                    id="template-component-font-size"
                    label="Font Size"
                  >
                    <Input
                      id="template-component-font-size"
                      ...
                      type="number"
                      value={numberValue(
                        selectedComponent.style?.font_size_pt,
                        10,
                      )}
                      onChange={(event) =>
                        updateSelectedStyle({
                  ...
                  <FormField id="template-component-align" label="Align">
                    <Select
                      id="template-component-align"
                      value={stringValue(
                        selectedComponent.style?.align,
                        "left",
                      )}
                      onChange={(event) =>
                        updateSelectedStyle({ align: event.target.value })
                      }
                    >
                  ...
                  {selectedComponent.type === "text" && (
                    <FormField id="template-component-text" label="Text">
                      <Input
                        ...
                        value={stringValue(selectedComponent.props?.text)}
                        onChange={(event) =>
                          updateSelectedProps({ text: event.target.value })
                        }
                      />
                    </FormField>
                  )}
                  {selectedComponent.type === "slip_number" && (
                    <FormField id="template-component-label" label="Label">
                      <Input
                        id="template-component-label"
                        value={stringValue(
                          selectedComponent.props?.label,
                          "Slip No",
                        )}
                        onChange={(event) =>
                          updateSelectedProps({ label: event.target.value })
                        }
                      />
                    </FormField>
                  )}
                  {selectedComponent.type === "divider" && (
                    ...
                    >
                      <Input
                        ...
                        type="number"
                        value={numberValue(
                          selectedComponent.props?.border_width_mm,
                          0.4,
                        )}
                        onChange={(event) =>
                          updateSelectedProps({
                            border_width_mm: Number(event.target.value),
                          })
                        }
                      />
                    </FormField>
                  ...
                  {(selectedComponent.type === "barcode" ||
                    ...
                    >
                      <Input
                        ...
                        type="number"
                        value={numberValue(
                          selectedComponent.props?.height_mm,
                          selectedComponent.type === "barcode" ? 10 : 4,
                        )}
                        onChange={(event) =>
                          updateSelectedProps({
                  ...
                  <Button onClick={removeSelectedComponent} variant="danger">
                    Remove Component
                  </Button>
                </div>
              ) : (
                <p className="template-editor-empty">
                  <LocalizedText text="Select a component on the canvas to update its coordinates and sizing." />
                </p>
              )}
            </Card>
          </div>
          ...
          <ActionBar>
            ...
            <Button onClick={clearActiveChanges} variant="secondary">
              Clear
            </Button>
            <Button
              isLoading={isPreviewing}
              onClick={() => void previewLayouts()}
              variant="secondary"
            >
              Preview
            </Button>
...
function createComponent(
  ...
  const style = {
    ...
    font_size_pt: 10,
    font_weight: "normal",
    width_percent: type === "divider" ? 92 : 40,
  };
  ...
  switch (type) {
    case "text":
      return {
        ...base,
        props: { ...base.props, text: "Pawn Loan Contract Slip" },
        style: {
          ...style,
          align: "center",
          font_size_pt: 14,
    ...
    case "tenant_logo":
      return {
        ...base,
        props: { ...base.props, width_mm: 24 },
        style: { ...style, width_percent: 18 },
      };
    case "slip_number":
    ...
    case "barcode":
      return {
        ...base,
        props: { ...base.props, height_mm: 10, show_text: true },
        style: { ...style, width_percent: 38 },
      };
    case "divider":
...
function isLayoutComponent(
  component: unknown,
): component is SlipLayoutComponent {
  return Boolean(
    component && typeof component === "object" && "type" in component,
  );
}
...
function componentPositionStyle(
  component: SlipLayoutComponent,
  paper: PaperPreset,
) {
  ...
  const width = clampNumber(
    numberValue(component.style?.width_percent, 100),
    10,
    100,
  );
  ...
  return {
    ...
    height:
      component.type === "spacer"
        ? `${Math.max(numberValue(component.props?.height_mm, 4), 1)}mm`
        : undefined,
    left: `${(x / paper.widthMm) * 100}%`,
    textAlign: stringValue(component.style?.align, "left") as
      "left" | "center" | "right",

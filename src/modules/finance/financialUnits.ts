...
export function isFinancialUnitCode(
  value: unknown,
): value is FinancialUnitCode {
...
export const financialUnitService = {
  list() {
    unitsRequest ??= apiClient
      .get<FinancialUnit[]>("/tenant/financial-units")
      .catch((error) => {
        unitsRequest = null;
        throw error;
      });
        ...
        if (active)
          setError(
            reason instanceof Error
              ? reason.message
              : "Unable to load financial units.",
          );
      })
      .finally(() => {
...
export function financialAmountToForm(
  ...
  if (!Number.isFinite(amount) || amount === 0)
    return { amount: amount ? String(amount) : "", unit: "UNIT" };
...
export function financialAmountToBase(
  value: FinancialAmountValue,
  units = defaultFinancialUnits,
) {
  const multiplier =
    units.find((unit) => unit.code === value.unit)?.multiplier ?? 1;
...
export function formatFinancialAmount(
  ...
  const configuredUnit = preferredUnit
    ? (units.find((item) => item.code === preferredUnit) ??
      defaultFinancialUnits.find((item) => item.code === preferredUnit))
    : null;
  const unit =
    configuredUnit ??

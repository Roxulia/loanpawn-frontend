...
export function formatAccountCurrencyAmount(
  ...
  const amount = Number(value ?? 0);
  const unit =
    Number.isFinite(amount) && Math.abs(amount) < 100_000 ? "UNIT" : null;
...
export function getStringField<TObject extends object>(
  item: TObject,
  snakeKey: string,
  camelKey: string,
) {
...
export function getNumberField<TObject extends object>(
  item: TObject,
  snakeKey: string,
  camelKey: string,
) {

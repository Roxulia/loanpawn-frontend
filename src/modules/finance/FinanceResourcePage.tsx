...
export type FinanceFormErrors<TForm extends FinanceFormState> = Partial<
...
export function FinanceResourcePage<TItem, TForm extends FinanceFormState>({
  ...
  const canCreate = config.createPermission
    ? hasPermission(config.createPermission)
    : false;
  const canUpdate = hasPermission(config.updatePermission);
  const canDelete = hasPermission(config.deletePermission);
  const canRenderItemActions =
    Boolean(config.renderItemActions) &&
    (!config.renderItemActionsPermission ||
      hasPermission(config.renderItemActionsPermission));
  const canUseRowActions =
    (canUpdate && !config.hideUpdateAction) ||
    canDelete ||
    canRenderItemActions;
      ...
      try {
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : `Unable to load ${config.totalLabel}.`,
        );
      } finally {
        setIsLoading(false);
      }
  ...
  function openCreateForm() {
    const createPath =
      config.mobileCreatePath && window.matchMedia("(max-width: 760px)").matches
  ...
  function openEditForm(item: TItem) {
    const editPath =
      config.mobileEditPath && window.matchMedia("(max-width: 760px)").matches
  ...
  async function handleSave() {
    ...
    try {
      ...
      setNotice(
        `${config.title} ${mode === "create" ? "created" : "updated"} successfully.`,
      );
      closeForm();
    } catch (saveError) {
      setError(
    ...
    try {
      ...
      setError(
        deleteError instanceof Error
          ? deleteError.message
          : `Unable to delete ${config.title.toLowerCase()}.`,
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  function updateLocalItem(nextItem: TItem) {
    ...
    setItems((current) =>
      current.map((item) =>
        config.getItemId(item) === nextItemId ? nextItem : item,
      ),
    );
  }
  ...
  function removeLocalItem(itemToRemove: TItem) {
    ...
    setItems((current) =>
      current.filter((item) => config.getItemId(item) !== itemId),
    );
          ...
          <DataTable
            ...
            emptyDescription={
              canList
                ...
                : `Your account cannot view ${config.totalLabel} records.`
            }
              ...
              config.renderMobileCard
                ? (item, actions) =>
                    config.renderMobileCard?.(
                      ...
                      actions,
                      config.detailPath
                        ? () => navigate(config.detailPath?.(item) ?? "")
                        : undefined,
                    )
                : undefined
            }
...
function getPageValue<TItem>(
  ...
  const withOptionalKeys = pageData as PaginatedResult<TItem> &
    Partial<Record<typeof camelKey | typeof snakeKey, number>>;

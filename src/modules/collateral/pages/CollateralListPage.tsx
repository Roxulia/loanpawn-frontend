...
export function CollateralListPage() {
  ...
  const loadItems = useCallback(
    async (page: number, search = debouncedSearchTerm) => {
      ...
      try {
        const response = await collateralService.listCollateral({
          page,
          perPage,
          search,
        });
        ...
        setError(
          loadError instanceof Error
            ? loadError.message
            : "Unable to load collateral items.",
        );
      } finally {
        setIsLoading(false);
      }
    ...
    try {
      ...
      setError(
        deleteError instanceof Error
          ? deleteError.message
          : "Unable to delete collateral item.",
      );
    } finally {
      setIsDeleting(false);
    }
  ...
  return (
    <section className="page ops-page ops-page--register">
      ...
      <Card
        ...
        description={`${total} total collateral item${total === 1 ? "" : "s"}`}
      >
        <div className="customer-management">
          ...
          <DataTable
            ...
            emptyDescription={
              canList
                ? debouncedSearchTerm
                  ? "No collateral items match this search."
                  : "No collateral items found."
                : "Your account cannot view collateral records."
            }
            emptyTitle={
              canList
                ? searchTerm
            ...
            onRowClick={
              canList
                ? (item) => navigate(routePaths.collateralDetail(item.code))
                : undefined
            }
            pagination={
              canList

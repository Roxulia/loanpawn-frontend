...
export const slipService = {
  ...
  createSlip(
    payload: SlipCreatePayload,
    auth?: TenantAuth,
    options: IdempotentRequestOptions = {},
  ) {
    const requestBody = payload.collateral_items.some(
      (item) => item.image_reference instanceof File,
    )
      ? slipPayloadToFormData(payload)
      : payload;

    return apiClient.post<LoanContractSlip>(
  ...
  getSlip(slipNo: string, auth?: TenantAuth) {
    return apiClient.get<LoanContractSlip>(
      `/tenant/loan-contract-slips/${encodeURIComponent(slipNo)}`,
      authOptions(auth),
    );
  },
  ...
  updateCompoundSchedule(
    ...
    return apiClient.put<LoanContractSlip>(
      `/tenant/loan-contract-slips/${encodeURIComponent(slipNo)}/compound-schedule`,
      payload,
      authOptions(auth),
    );
  },
  ...
  compoundInterest(slipNo: string, auth?: TenantAuth) {
    return apiClient.post<{
      ...
      `/tenant/loan-contract-slips/${encodeURIComponent(slipNo)}/compound-interest`,
      {},
      authOptions(auth),
    );
  },

  collectPartialPrincipal(
    slipNo: string,
    payload: {
      slip_update_key: number;
      amount: number;
    ...
    return apiClient.post<{
      ...
      `/tenant/loan-contract-slips/${encodeURIComponent(slipNo)}/partial-principal`,
      payload,
      authOptions(auth),
    );
  },
  ...
  listInterestTypes(auth?: TenantAuth) {
    return apiClient.get<InterestType[]>(
  ...
  listMaterialTypes(auth?: TenantAuth) {
    return apiClient.get<MaterialType[]>(
  ...
  listItemCategoryTypes(auth?: TenantAuth) {
    return apiClient.get<ItemCategoryType[]>(

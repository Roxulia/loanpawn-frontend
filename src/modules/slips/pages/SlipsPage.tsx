...
export function SlipsPage() {
  ...
  const [isCustomerInfoRequired, setIsCustomerInfoRequired] = useState<
    boolean | null
  >(null);
  ...
  const loadSlips = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        setSlips(nextItems);
        setCurrentPage(
          getPageValue(pageData, "currentPage", "current_page", page),
        );
  ...
  useEffect(() => {
    ...
    if (
      ...
      requestedCustomerCode === prefilledCustomerCode
    ) {
      return;
    }
    ...
    const loadTimer = window.setTimeout(() => {
      ...
      void customerService
        .getCustomer(requestedCustomerCode)
        .then((response) => {
          if (!isCurrent) {
            return;
          }
  ...
  const columns: Array<DataTableColumn<LoanContractSlip>> = [
    {
      header: "Slip No",
      key: "slipNo",
      render: (slip) => <strong>{slip.slip_no}</strong>,
    },
    { header: "Customer", key: "customer", render: getSlipCustomerName },
    {
      ...
      render: (slip) => (
        <AccountCurrencyAmount
          accountId={slip.account_id ?? slip.accountId}
    ...
    },
    {
      header: "Created",
      key: "created",
      render: (slip) => formatDate(slip.created_at),
    },
  ];
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    const nextErrors = validateSlipForm(
      customer,
      loan,
      items,
      interestTypes,
      ...
      const response = await slipService.createSlip(
        {
          ...
          ...(loan.reporting_exchange_rate
            ? {
                reporting_exchange_rate: Number(loan.reporting_exchange_rate),
                reporting_exchange_rate_inversed:
                  loan.reporting_exchange_rate_inversed,
              }
            : {}),
  ...
  async function printSlipDocument(
  ...
  const jewelleryItemCount = items.filter(
    (item) => item.type === "Jewellery",
  ).length;
    ...
    <section className="page ops-page ops-page--slips">
      <div className="ops-hero">
        ...
        <div
          ...
          aria-label={t("Loan slip workspace summary")}
        >
          <div className="ops-metric">
            <span>Monthly usage</span>
            <strong>
              <ResourceUsageBadge resource="slips" />
            </strong>
          </div>
          <div className="ops-metric">
      ...
      <div
        ...
        role="tablist"
        aria-label={t("Loan slip sections")}
      >
        <Button
          aria-pressed={activeTab === "application"}
          onClick={() => setActiveTab("application")}
          variant={activeTab === "application" ? "primary" : "secondary"}
        >
          Loan Application
        </Button>
        <Button
      ...
      {activeTab === "application" && !isLoanSlipCreationSettingsLoading && (
        <form
          className="workflow-stack ops-contract-workspace"
        ...
        >
                ...
                <FormGroup className="slip-form-customer-grid" columns={2}>
                  ...
                  >
                    <Input
                      ...
                      hasError={Boolean(formErrors.customerName)}
                    />
                  </FormField>
                  ...
                  <FormField
                    id="slip-customer-nrc"
                    label="NRC"
                    error={formErrors.customerNrc}
                  >
                    <NrcField
                      id="slip-customer-nrc"
                      value={customer.nrc}
                      onChange={(nrc) => setCustomer({ ...customer, nrc })}
                      hasError={Boolean(formErrors.customerNrc)}
                    />
                  </FormField>
                  <FormField id="slip-customer-email" label="Email">
                    <Input
                      id="slip-customer-email"
                      type="email"
            ...
            <div className="workflow-stack">
              ...
              {items.map((item, index) => (
                <section className="subform-panel" key={item.key}>
                  ...
                  >
                    ...
                    >
                      <Input
                        ...
                        hasError={Boolean(formErrors[`${item.key}.name`])}
                      />
                    </FormField>
                    {item.type === "Normal" && (
                    ...
                    {item.type === "Normal" && (
                      <FormField
                        id={`${item.key}-item-category`}
                        label="Item Category"
                      >
                        <Select
                          id={`${item.key}-item-category`}
                          value={item.item_category_type_id ?? ""}
                          onChange={(event) =>
                            updateItem(item.key, {
                    ...
                    <FormField
                      ...
                      error={formErrors[`${item.key}.image_reference`]}
                    >
                      <ExpenseImageInput
                        existingImage={false}
                        ...
                        onChange={(file) =>
                          updateItem(item.key, {
                            image_reference: file ?? undefined,
                          })
                        }
                  ...
                  {item.type === "Normal" && (
                    <FormGroup
                      className="slip-form-collateral-summary-grid"
                      columns={3}
                    >
                      <FormField
                        id={`${item.key}-estimated`}
                      ...
                      >
                        <FinancialAmountInput
                          ...
                          min="0"
                          value={{
                            amount: String(item.estimated_value ?? ""),
                            unit: item.estimated_value_unit ?? "UNIT",
                          }}
                          onChange={(next) =>
                            updateItem(item.key, {
                              estimated_value: Number(next.amount),
                              estimated_value_unit: next.unit,
                            })
                          }
                        />
                      </FormField>
                  ...
                  {item.type === "Jewellery" && (
                    <>
                      ...
                      >
                        ...
                        <FormField
                          id={`${item.key}-material-price`}
                          label="Material Price per Kyat"
                        >
                          <FinancialAmountInput
                            id={`${item.key}-material-price`}
                            min="0"
                            value={{
                              amount: String(
                                item.material_price_per_kyat ?? "",
                              ),
                              unit: item.material_price_per_kyat_unit ?? "UNIT",
                            }}
                            onChange={(next) =>
                              updateItem(item.key, {
                                material_price_per_kyat: Number(next.amount),
                                material_price_per_kyat_unit: next.unit,
                              })
                            }
                          />
                        </FormField>
                      ...
                      <FormGroup
                        className="slip-form-jewellery-weight-grid"
                        columns={3}
                        title="Weight"
                      >
                        <FormField id={`${item.key}-kyat`} label="Kyat">
                          <Input
                            ...
                            onChange={(event) =>
                              updateItem(item.key, {
                                kyat: Number(event.target.value),
                              })
                            }
                          />
                        </FormField>
                        ...
                        <FormField id={`${item.key}-yway`} label="Yway">
                          <Input
                            ...
                            onChange={(event) =>
                              updateItem(item.key, {
                                yway: Number(event.target.value),
                              })
                            }
                          />
                        </FormField>
                  ...
                  {item.type === "Jewellery" && (
                    <div className="workflow-stack">
                      <label className="checkbox-line">
                        ...
                        />
                        <span>
                          <LocalizedText text="Have Gem Stone" />
                        </span>
                      </label>
                      {item.contains_gemstones && (
                        <FormGroup
                          className="slip-form-gemstone-grid"
                          columns={2}
                        >
                          <FormField
                            id={`${item.key}-gemstone-type`}
                            label="Gemstone Type"
                          >
                            <Input
                              id={`${item.key}-gemstone-type`}
                              ...
                              onChange={(event) =>
                                updateItem(item.key, {
                                  gemstone_type: event.target.value,
                                })
                              }
                            />
                          </FormField>
                          <FormField
                            id={`${item.key}-gemstone-weight`}
                          ...
                          >
                            <Input
                              ...
                              onChange={(event) =>
                                updateItem(item.key, {
                                  gemstone_weight: event.target.value,
                                })
                              }
                            />
                          </FormField>
                          <FormField
                            id={`${item.key}-gemstone-quantity`}
                          ...
                          <FormField
                            id={`${item.key}-gemstone-grade`}
                            label="Gemstone Grade"
                          >
                            <Input
                              id={`${item.key}-gemstone-grade`}
                              value={item.gemstone_grade ?? ""}
                              onChange={(event) =>
                                updateItem(item.key, {
                                  gemstone_grade: event.target.value,
                                })
                              }
                            />
                          </FormField>
                  ...
                  {item.type === "Jewellery" && (
                    <FormGroup
                      className="slip-form-collateral-summary-grid slip-form-collateral-summary-grid--jewellery"
                      columns={2}
                    >
                      <FormField id={`${item.key}-quantity`} label="Quantity">
                        <Input
          ...
          <Card
            title="Loan Details"
            description={`Suggested minimum retail total: ${formatMoney(suggestedMinimumRetail)}`}
          >
            <FormGroup className="slip-form-loan-grid" columns={3}>
              <FormField
                ...
                helperText="The loan and its interest use this account's currency."
              >
                <FinancialAccountSelect
                  hasError={Boolean(formErrors.accountId)}
                  id="loan-account"
                  onChange={(accountId) =>
                    setLoan({ ...loan, account_id: accountId })
                  }
              ...
              <ReportingExchangeRateField
                ...
                inversed={loan.reporting_exchange_rate_inversed}
                manualRate={loan.reporting_exchange_rate}
                onInversedChange={(inversed) =>
                  setLoan((current) => ({
                    ...current,
                    reporting_exchange_rate_inversed: inversed,
                  }))
                }
                onManualRateChange={(rate) =>
                  setLoan((current) => ({
                    ...current,
                    reporting_exchange_rate: rate,
                  }))
                }
              />
              <FormField
              ...
              >
                <FinancialAmountInput
                  ...
                  value={{
                    ...
                    unit: loan.loan_amount_unit,
                  }}
                  onChange={(next) =>
                    setLoan({
                      ...loan,
                      loan_amount: next.amount,
                      loan_amount_unit: next.unit,
                    })
                  }
                  hasError={Boolean(formErrors.loanAmount)}
                />
              </FormField>
              <FormField
                id="interest-rate"
              ...
              >
                <Input
                  ...
                  onChange={(event) =>
                    setLoan({ ...loan, interest_rate: event.target.value })
                  }
                  hasError={Boolean(formErrors.interestRate)}
                />
              </FormField>
              <FormField
                id="interest-type"
                label="Interest Type"
                error={formErrors.interestType}
              >
                <Select
                  id="interest-type"
                  value={loan.interest_type_id}
                  onChange={(event) =>
                    setLoan({ ...loan, interest_type_id: event.target.value })
                  }
                  hasError={Boolean(formErrors.interestType)}
                >
              ...
              <FormField
                id="expiry-quota"
                label="Expiry Quota"
                error={formErrors.expiryQuota}
              >
                <Input
                  id="expiry-quota"
                  min="1"
                  type="number"
                  value={loan.expiry_quota}
                  onChange={(event) =>
                    setLoan({ ...loan, expiry_quota: event.target.value })
                  }
                  hasError={Boolean(formErrors.expiryQuota)}
                />
              </FormField>
              <FormField id="expiry-type" label="Expiry Unit">
                <Select
                  id="expiry-type"
                  value={loan.expiry_quota_type}
                  onChange={(event) =>
                    setLoan({ ...loan, expiry_quota_type: event.target.value })
                  }
                >
              ...
              >
                <Textarea
                  ...
                  value={loan.notes}
                  onChange={(event) =>
                    setLoan({ ...loan, notes: event.target.value })
                  }
                />
              </FormField>
            </FormGroup>
            <ActionBar>
              <label className="checkbox-line">
                <input
                  checked={shouldPrintAfterCreate}
                  onChange={(event) =>
                    setShouldPrintAfterCreate(event.target.checked)
                  }
                  type="checkbox"
                />
                <span>
                  <LocalizedText text="Print after saving" />
                </span>
              </label>
              ...
              <Button
                disabled={!canCreate || isCustomerInfoRequired === null}
      ...
      {activeTab === "management" && (
        <div className="workflow-stack ops-register-workspace">
          <Card
            title="All Slips"
            description={`${total} total slip${total === 1 ? "" : "s"}`}
          >
            <TableToolbar
              actions={
                <Button
                  disabled={!canList}
                  onClick={() => void loadSlips(currentPage)}
                  variant="secondary"
                >
                  Refresh
                </Button>
            ...
            <DataTable
              ...
              emptyDescription={
                searchTerm
                  ? "No slips match this search."
                  : "Create the first loan contract slip."
              }
...
function RetailPriceField({ item }: { item: ItemForm }) {
  return (
    <div className="ui-form-field slip-form-retail-field">
      ...
      <Badge tone="info">
        {formatMoney(calculateMinimumRetailPrice(item))}
      </Badge>
      <div className="ui-form-field__hint">
        <LocalizedText text="Calculated from item value, quantity, and jewellery weight where applicable." />
      </div>
    </div>
  );
}
...
function validateSlipForm(
  ...
  if (customerInfoRequired) {
    ...
    if (
      isEmptyNrcValue(customer.nrc) &&
      !customer.email.trim() &&
      !customer.phone.trim()
    ) {
      errors.customerName =
        errors.customerName ?? "One of NRC, email, or phone is required.";
    }
  }
  ...
  items.forEach((item) => {
    ...
    if (item.image_reference && item.image_reference.size > 5 * 1024 * 1024) {
      errors[`${item.key}.image_reference`] =
        "Reference image must not exceed 5 MB.";
    }

    if (
      item.image_reference &&
      !["image/jpeg", "image/png", "image/webp"].includes(
        item.image_reference.type,
      )
    ) {
      errors[`${item.key}.image_reference`] =
  ...
  const interestType = interestTypes.find(
    (type) => type.id === Number(loan.interest_type_id),
  );
...
function calculateExpiryDurationInDays(
  quota: number,
  quotaType: string,
  ...
  return {
    ...
    material_price_per_kyat_unit:
      item.type === "Jewellery"
        ...
        : undefined,
    item_category_type_id:
      item.type === "Normal" ? item.item_category_type_id : undefined,
  ...
  return (
    <article className="slip-history-mobile-card">
      ...
      <footer className="slip-history-mobile-card__footer">
        <div className="slip-history-mobile-card__actions row-actions">
          {actions}
        </div>
        <button
          className="slip-history-mobile-card__details"
          onClick={onView}
          type="button"
        >
          View Details
...
function calculateMinimumRetailPrice(item: ItemForm) {
  ...
  return roundMoney(
    financialAmountToBase({
      amount: String(item.estimated_value ?? 0),
      unit: item.estimated_value_unit ?? "UNIT",
    }) * Number(item.quantity ?? 1),
  );
}
...
function calculateJewelleryWeightInKyat(item: ItemForm) {
  return (
    Number(item.kyat ?? 0) +
    Number(item.pal ?? 0) / 16 +
    Number(item.yway ?? 0) / 128
  );
}
